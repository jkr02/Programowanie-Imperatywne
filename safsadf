...
void char_count(int char_no, int *n_char, int *cnt){
	...
	for (int i=0; i<LAST_CHAR-FIRST_CHAR; i++){
		koniec=0;
		for(int j=i; j<LAST_CHAR-FIRST_CHAR-i; i++){
			if(tab[j] > tab[j+1]){
				koniec=1;
				tmp=tab[j];
				tab[j] = tab[j+1];
	...
	*n_char=tab[char_no-1];
	*cnt=indeksy[char_no-1];
}
...
void digram_count(int digram_no, int digram[]){
	int pop=fgetc(stdin);
	int text=0;
	int indeksy[MAX_DIGRAMS];
	for(int i =0; i<(LAST_CHAR - FIRST_CHAR)*(LAST_CHAR - FIRST_CHAR); i++){
		indeksy[i]=i;
	}
	while((text=fgetc(stdin))!=EOF){
		count[(pop-FIRST_CHAR)*(LAST_CHAR - FIRST_CHAR)+text-FIRST_CHAR)]+=1
	}
	
	digram[0]=(count[digram_no]+FIRST_CHAR)/(LAST_CHAR - FIRST_CHAR)+FIRST_CHAR;
	digram[1]=(count[digram_no]+FIRST_CHAR)%(LAST_CHAR - FIRST_CHAR);
}
